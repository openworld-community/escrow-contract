...
def __init__(
    ...
    assert msg.sender == _buyer or msg.sender == _seller
...
def approve_deal_terms():
    ...
    """
    assert self.status == Status.WAIT_SELLER_APPROVE and msg.sender == self.seller
    self.status = Status.WAIT_BUYER_DEPOSIT
...
def deposit():
    ...
    """
    assert self.status == Status.WAIT_BUYER_DEPOSIT and msg.sender == self.buyer
    assert msg.value >= self.value
...
def confirm_service():
    ...
    """
    assert self.status == Status.WAIT_SELLER_SERVICE and msg.sender == self.seller
...
def approve_receipt():
    ...
    """
    assert self.status == Status.WAIT_BUYER_APPROVE and msg.sender == self.buyer
...
def refund_to_buyer():
    ...
    """
    assert self.status == Status.DISPUT and msg.sender == self.arbiter
...
def withdraw():
    ...
    """
    assert msg.sender == self.seller and self.expired_at <= block.timestamp
    assert self.status == Status.WAIT_BUYER_APPROVE or self.status == Status.DISPUT
